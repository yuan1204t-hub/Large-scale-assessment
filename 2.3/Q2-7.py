...
4. For M6:
   Hyperparameters are tuned within the LOOCV procedure according to the manuscript method:
   standardized predictors, grid search over kernel structure / alpha / normalize_y,
   and GaussianProcessRegressor kernel-parameter optimization using fmin_l_bfgs_b with
...
# M6 no longer reads a parameter summary file in this manuscript-consistent version.
# M6 hyperparameters are tuned within each LOOCV training fold.
...
# =========================================================
# 8. M6: manuscript-consistent Gaussian process regression
# =========================================================
# According to the manuscript:
# - Input features are standardized before GPR fitting.
# - Kernel-parameter optimization uses fmin_l_bfgs_b.
# - n_restarts_optimizer is set to 2.
# - Hyperparameter tuning focuses on kernel structure, alpha, and normalize_y.
# - For LOOCV, tuning/scaling/fitting are repeated within each training fold.

M6_ALPHA_GRID = [1e-10, 1e-6, 1e-4, 1e-2]
M6_NORMALIZE_Y_GRID = [True, False]


def build_m6_kernel_candidates():
    """
    Build candidate covariance kernels for M6.

    Candidate kernels include a ConstantKernel combined with RBF, Matern, or
    RationalQuadratic, as well as corresponding forms combined with WhiteKernel.
    Finite parameter bounds are used so that n_restarts_optimizer can work.
    """
    base_candidates = [
        (
            "ConstantKernel*RBF",
        ...
        (
            ...
            C(1.0, constant_value_bounds=(1e-3, 1e3))
            * Matern(length_scale=1.0, nu=1.5, length_scale_bounds=(1e-3, 1e3))
        ),
        (
            "ConstantKernel*Matern(nu=2.5)",
            C(1.0, constant_value_bounds=(1e-3, 1e3))
            * Matern(length_scale=1.0, nu=2.5, length_scale_bounds=(1e-3, 1e3))
        ),
        (
    ...
    for kernel_name, kernel_obj in base_candidates:
        candidates.append((kernel_name, kernel_obj))
        candidates.append((
            f"{kernel_name}+WhiteKernel",
            kernel_obj + WhiteKernel(
                noise_level=1.0,
...
def build_m6_model(kernel_obj, alpha, normalize_y):
    """Build M6 model with standardized predictors and optimized GPR kernel parameters."""
...
def evaluate_m6_params_by_loocv(X_values, y_values, kernel_obj, alpha, normalize_y):
    """
    Evaluate one M6 hyperparameter combination using inner LOOCV.

    The CV error used for grid search is mean squared prediction error.
    Scaling and GPR fitting are performed inside the pipeline for each fold.
    """
    ...
    for inner_train_idx, inner_test_idx in loo.split(X_values):
        X_inner_train = X_values[inner_train_idx]
        X_inner_test = X_values[inner_test_idx]
        y_inner_train = y_values[inner_train_idx]
        y_inner_test = y_values[inner_test_idx]
        ...
        try:
            ...
            y_true_all.append(float(y_inner_test[0]))
            y_pred_all.append(float(y_pred[0]))
    ...
    return mean_squared_error(y_true_all, y_pred_all)


def tune_m6_hyperparameters(X_train, y_train):
    """
    Tune M6 hyperparameters by grid search combined with inner LOOCV.
    ...
    for kernel_name, kernel_obj in build_m6_kernel_candidates():
        for alpha in M6_ALPHA_GRID:
            for normalize_y in M6_NORMALIZE_Y_GRID:
                score = evaluate_m6_params_by_loocv(
                    X_values=X_train,
                    y_values=y_train,
    ...
    if best_info is None:
        raise ValueError("M6 hyperparameter tuning failed for all candidate settings.")

    return best_info


def run_nested_loocv_m6(X, y):
    """
    Run nested LOOCV for M6.

    For each outer LOOCV split:
    1. Hold out one sample.
    2. Tune kernel / alpha / normalize_y using only the outer training subset.
    3. Fit the selected GPR pipeline on the outer training subset.
    ...
    for outer_train_idx, outer_test_idx in loo.split(X_values):
        X_outer_train = X_values[outer_train_idx]
        X_outer_test = X_values[outer_test_idx]
        y_outer_train = y_values[outer_train_idx]
        y_outer_test = y_values[outer_test_idx]

        best_info = tune_m6_hyperparameters(X_outer_train, y_outer_train)

        model = build_m6_model(
            kernel_obj=best_info["kernel_obj"],
            alpha=best_info["alpha"],
            normalize_y=best_info["normalize_y"]
        )
        ...
        y_true_all.append(float(y_outer_test[0]))
        y_pred_all.append(float(y_pred[0]))

        outer_best_settings.append({
            "kernel_name": best_info["kernel_name"],
            "alpha": best_info["alpha"],
            "normalize_y": best_info["normalize_y"],
            "inner_cv_mse": best_info["inner_cv_mse"]
        })

    return np.array(y_true_all), np.array(y_pred_all), outer_best_settings


def summarize_m6_outer_settings(outer_best_settings):
    """Summarize fold-specific M6 hyperparameter selections."""
    if len(outer_best_settings) == 0:
        return {
            "LOOCV_Mode_Kernel": np.nan,
            "LOOCV_Mode_Alpha": np.nan,
            "LOOCV_Mode_Normalize_Y": np.nan,
            "LOOCV_Mode_Count": 0
        }
    ...
    setting_tuples = [
        ...
        for item in outer_best_settings
    ]

    mode_setting, mode_count = Counter(setting_tuples).most_common(1)[0]

    return {
        "LOOCV_Mode_Kernel": mode_setting[0],
        "LOOCV_Mode_Alpha": mode_setting[1],
        "LOOCV_Mode_Normalize_Y": mode_setting[2],
        "LOOCV_Mode_Count": mode_count
    }


def fit_m6_full_data_with_tuning(X, y):
    """
    Tune M6 on the full dataset using LOOCV, then refit the selected model on full data.
    """
    X_values = np.asarray(X, dtype=float)
    y_values = np.asarray(y, dtype=float)

    best_info = tune_m6_hyperparameters(X_values, y_values)
    ...
    optimized_kernel = str(model.named_steps["m6"].kernel_)

    refit = {
        "R2_refit": r2_score(y_values, y_pred),
        "RMSE_refit": np.sqrt(mean_squared_error(y_values, y_pred)),
        "MAE_refit": mean_absolute_error(y_values, y_pred)
    }

    return refit, best_info, optimized_kernel


def run_m6_for_one_file(file_path):
    """Run manuscript-consistent nested LOOCV Q2 calculation for M6."""
    X, y, x_names, y_name = read_dataset(file_path)

    y_true, y_pred, outer_best_settings = run_nested_loocv_m6(
        X=X.values,
        y=y.values
    )

    q_metrics = calculate_q2_metrics(y_true, y_pred)
    refit, full_best_info, optimized_kernel = fit_m6_full_data_with_tuning(
        X=X.values,
        y=y.values
    )
    outer_summary = summarize_m6_outer_settings(outer_best_settings)
    ...
    return {
        ...
        "n_original_features": X.shape[1],
        "n_model_terms": X.shape[1],
        "Hyperparameters_or_Selected_Terms": (
            "Nested LOOCV tuning; "
            f"full-data best kernel={full_best_info['kernel_name']}; "
            f"alpha={full_best_info['alpha']}; "
            f"normalize_y={full_best_info['normalize_y']}"
        ),
        "Best_Kernel": full_best_info["kernel_name"],
        "Best_Alpha": full_best_info["alpha"],
        "Best_Normalize_Y": full_best_info["normalize_y"],
        "Full_Data_Inner_CV_MSE": full_best_info["inner_cv_mse"],
        "Full_Data_Optimized_Kernel": optimized_kernel,
        **outer_summary,
        **q_metrics,
        **refit
    }
...
def run_m6_batch(file_map):
    """Run M6 batch calculation with manuscript-consistent nested tuning."""
    results = []
    errors = []
    ...
    for file_name, file_path in file_map.items():
        try:
            ...
            summary = run_m6_for_one_file(file_path=file_path)
            results.append(summary)
