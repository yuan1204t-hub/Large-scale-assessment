...
3. Perform the Breusch-Pagan heteroscedasticity test for three traditional regression models:
...
4. Output separate sheets for M0, M1, M2, a combined long-format sheet,
...
Notes:
- BP_LM_pvalue is usually the main Breusch-Pagan p-value.
- BP_F_pvalue is also exported as a supplementary version.
"""
...
# =========================
# 6. Fit OLS and run Breusch-Pagan test
# =========================
def fit_ols_and_bp_test(X, y):
    """
    Fit an OLS model and perform the Breusch-Pagan heteroscedasticity test.
    """
...
# =========================
# 8. Calculate M1 or M2 BP for one dataset
# =========================
def calculate_selected_model_for_one_dataset(file_path, best_variables_text, model_name):
    """
    Calculate Breusch-Pagan test results for M1 or M2 according to selected variables.
    """
    df, X_raw, y, y_col, x_cols = read_original_dataset(file_path)
    ...
    metrics = fit_ols_and_bp_test(X_selected, y)
...
def run_m0(file_list):
    ...
    print("\n[INFO] Running M0 Breusch-Pagan test...")
    ...
    for idx, file_path in enumerate(file_list, start=1):
        ...
        try:
            ...
            print(f"  -> Success: BP_LM_pvalue = {metrics['BP_LM_pvalue']:.6f}")
...
def run_selected_model(result_file, model_name):
    """
    Run Breusch-Pagan test for M1 or M2 based on the selected variables
    reported in the corresponding summary file.
    """
    results = []
    errors = []

    print(f"\n[INFO] Running {model_name} Breusch-Pagan test...")
...
# =========================
def main():
    print("[INFO] Starting comprehensive Breusch-Pagan test for M0, M1, and M2...")
    ...
    if not all_results_df.empty:
        ...
        # Add a simple pass/fail indicator based on BP_LM_pvalue >= 0.05
        for model_name in ["M0", "M1", "M2"]:
            p_col = f"BP_LM_pvalue_{model_name}"
            pass_col = f"BP_Pass_LM_0.05_{model_name}"

            if p_col in comparison_wide_df.columns:
                comparison_wide_df[pass_col] = comparison_wide_df[p_col] >= 0.05
    else:
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        all_results_df.to_excel(writer, sheet_name="All_BP_Long", index=False)
        comparison_wide_df.to_excel(writer, sheet_name="BP_Comparison_Wide", index=False)
        m0_df.to_excel(writer, sheet_name="M0_BP", index=False)
        m1_df.to_excel(writer, sheet_name="M1_BP", index=False)
        m2_df.to_excel(writer, sheet_name="M2_BP", index=False)
    ...
    print(f"[INFO] Comprehensive Breusch-Pagan results saved to: {output_file}")
