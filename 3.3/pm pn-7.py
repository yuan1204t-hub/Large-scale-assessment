...
"""
Purpose:
1. Batch calculate residual diagnostic metrics for seven models:
   - M0
   - M1
   - M2
   - M3
...
2. Diagnostic metrics include:
   - pn: p-value of the Shapiro-Wilk normality test for residuals
   - pm: p-value of the one-sample t test for residual mean = 0
   - Residual_Mean
   - Residual_SD
   - Max_Abs_Residual
   - AE_Q1
...
5. Output:
   - All_Diagnostics_Long
   - Diagnostics_Comparison_Wide
   - M0_Diagnostics
   - M1_Diagnostics
   - M2_Diagnostics
   - M3_Diagnostics
...
- All diagnostics are calculated from full-data refit residuals.
"""
...
def calculate_pn_pm(residuals):
    ...
    pm:
    p-value of the one-sample t test for residual mean = 0.
    """
    ...
    if len(residuals) < 2:
        ...
        try:
            pm = ttest_1samp(residuals, popmean=0).pvalue
        except Exception:
            pm = np.nan
...
def calculate_residual_diagnostics(residuals):
    ...
    pn, pm = calculate_pn_pm(residuals)

    residual_mean = np.mean(residuals)
    residual_sd = np.std(residuals, ddof=1) if len(residuals) > 1 else np.nan
    ...
    return {
        ...
        "Max_Abs_Residual": np.max(abs_error),
        "AE_Q1": ae_q1,
        "AE_Q3": ae_q3,
        "AE_IQR": ae_iqr,
...
def get_model_fit_metrics(y_true, y_pred):
    """Calculate simple full-data fitting metrics."""
...
def run_m0_for_one_file(file_path):
    """Run residual diagnostics for M0."""
    ...
    diagnostics = calculate_residual_diagnostics(residuals)
    fit_metrics = get_model_fit_metrics(y.values, y_pred.values)
...
def run_selected_ols_for_one_file(file_path, best_variables_text, model_name):
    """Run residual diagnostics for M1 or M2."""
    X_raw, y = read_dataset(file_path)
...
def run_m3_for_one_file(file_path, alpha, fit_intercept=True):
    """Run residual diagnostics for M3."""
    ...
    diagnostics = calculate_residual_diagnostics(residuals)
    fit_metrics = get_model_fit_metrics(y.values, y_pred)
...
def run_m4_for_one_file(file_path, kernel, c_value, epsilon, gamma):
    """Run residual diagnostics for M4."""
    X, y = read_dataset(file_path)
...
def run_m5_for_one_file(file_path, n_components):
    """Run residual diagnostics for M5."""
    X, y = read_dataset(file_path)
...
def run_m6_for_one_file(file_path, kernel_obj, kernel_str, alpha, normalize_y):
    """Run residual diagnostics for M6."""
    X, y = read_dataset(file_path)
...
# =========================================================
def main():
    print("[INFO] Starting seven-model residual diagnostics integration...")
    ...
    if not all_results_df.empty:
        ...
        for model_name in model_order:
            pn_col = f"pn_{model_name}"
            pm_col = f"pm_{model_name}"

            if pn_col in comparison_wide_df.columns:
                comparison_wide_df[f"pn_Pass_0.05_{model_name}"] = comparison_wide_df[pn_col] >= 0.05

            if pm_col in comparison_wide_df.columns:
                comparison_wide_df[f"pm_Pass_0.05_{model_name}"] = comparison_wide_df[pm_col] >= 0.05
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        all_results_df.to_excel(writer, sheet_name="All_Diagnostics_Long", index=False)
        comparison_wide_df.to_excel(writer, sheet_name="Diagnostics_Comparison_Wide", index=False)
        ...
        for model_name in model_order:
            ...
            model_df.to_excel(writer, sheet_name=f"{model_name}_Diagnostics", index=False)

        all_errors_df.to_excel(writer, sheet_name="Errors", index=False)

    print("\n[INFO] Seven-model residual diagnostics integration completed.")
